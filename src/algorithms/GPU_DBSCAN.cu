...
__device__
float euclidian_distance(int i_point, int j_point, float *d_data, int d) {
    float *p = &d_data[i_point * d];
    float *q = &d_data[j_point * d];
    double distance = 0;
    for (int i_dimension = 0; i_dimension < d; i_dimension++) {
        double diff = p[i_dimension] - q[i_dimension];
        distance += diff * diff;
    }
...
__global__
void kernel_neighborhood_sizes(int *d_neighborhood_sizes, float *d_data, int n, int d, float eps) {
